...
/// </summary>
public sealed class WebProjectArchive
{
    ...
    public WebProjectArchive(IndexedDbInterop idb, WebFileDownloadInterop files)
    {
        _Idb   = idb;
        _Files = files;
    }
    ...
    public async Task ExportAsync(string location, string suggestedFileName)
    {
        ...
        using (MemoryStream buffer = new MemoryStream())
        {
            ...
            {
                foreach (string path in await _Idb.ListAllAsync(location))
                {
                    string? content = await _Idb.GetAsync(location, path);
        ...
        await _Files.SaveBytesAsync(suggestedFileName, zipBytes, _ZIP_MIME);
    }
    ...
    {
        byte[]? zipBytes = await _Files.PickBytesAsync();
        ...
        using (ZipArchive zip = new ZipArchive(buffer, ZipArchiveMode.Read))
        {
            foreach (ZipArchiveEntry entry in zip.Entries)
            {
                ...
                await _Idb.PutAsync(location, path, content);
        ...
        if (!hasMeta)
        {
            ...
            await _Idb.DeleteLocationAsync(location);
            return null;
        }
